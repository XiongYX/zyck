...
<template>
    <div>
        <div class="tzy-seach-warp">
            <check-list list-name="理想院校：" :list="lxxxList" :has-select="true" label-width="110px">
                <template v-for="item in lxxxList">
                    <Tag style="margin: 10px 10px 10px 0;" class="zyck-tag">{{item.value}}</Tag>
                </template>
                <Button type="text" style="color: #2689E6;height: 30px;line-height: 30px;margin: 10px 10px 10px 0px;" @click="lxyxShow = true">
                    <template v-if="lxxxList.length>0">修改</template>
                    <template v-else>添加</template>
                </Button>
                <template v-if="lxxxList.length>0">
            ...
            <div style="padding: 10px 0; border-top: 1px solid #e9eaec; text-align: center;">
                <Button style="width: 180px" size="large" type="primary" @click="buildList(true)">按以上条件筛选院校</Button>
            </div>
        </div>
        ...
        <zy-table :table-list="tableList" :years="years" :xxyx="postData.xxyx" @zy-expend="expend" @show-yxmyd="showYxmyd" @show-zymyd="showZymyd" @add-zy="addZy"></zy-table>
        ...
        <template v-if = "lxyxShow">
            <lxyx :modalClosable="true" :editYx="true" @closeyx="closeyx" @on-updata="updataYx"></lxyx>
        </template>
...
<script>
    import cxtj from "../../../data/cxtj-yxfirst.json";
    ...
    export default {
        ...
        computed: {
            lxxxList () {
                return this.$store.state.userMsg.lxyx;
            },
            tzydisabled () {
        ...
        methods: {
            ...
            postZyMessage () {
                postZhiy(this.zyData).then(({data}) => {
                    if(data.flag) {
                        ...
                        this.$refs['yzList'].showList(false);
                    } else {
            ...
            expend ({ index, xxdm, fxlx, flag, zy}) {
                let zylist = [];
                zy.forEach((zyitem) => {
                    zylist.push(zyitem.zydm+'_'+zyitem.lqfx.key);
                });
